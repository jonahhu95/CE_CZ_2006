    ...
    public class DatabaseManager
    {
        ...
        public Boolean saveFeedback(Feedback feedback)
        {
            try
            {
                if (getConnection())
                {
                    string sqlStatement = "INSERT INTO FEEDBACK(user_name,message) VALUES (" +
                        feedback.getSubmitter() + "," + feedback.getMessage() + ");";
                    return insertToTable(sqlStatement);
                }
        ...
        public Boolean saveCalculation(Calculation calculation, User user)
        {
            try
            {
                if (getConnection())
                {
                    string sqlStatement = "INSERT INTO CALCULATION(user_name,created_time,work_location_lon," +
                        "work_location_lat,work_location_name,work_location_area,home_location_lon,home_location_lat," +
                        "home_location_name,home_location_area,salary,commute_type,job_interest,salary_satisfaction," +
                        "median_salary,riders_area,ave_riders_area,commute_time,ave_commute_time,monthly_commute_cost) VALUES (" +
                        user.getUsername() + "," + calculation.getCreatedTime() + "," + calculation.getWorkLocation().getLongitude() + "," +
                        calculation.getWorkLocation().getLatitude() + "," + calculation.getWorkLocation().getLocationName() + "," +
                        calculation.getWorkLocation().getArea() + "," + calculation.getHomeLoction().getLongitude() + "," +
                        calculation.getHomeLoction().getLatitude() + "," + calculation.getHomeLoction().getLocationName() + "," +
                        calculation.getHomeLoction().getArea() + "," + calculation.getSalary() + "," + calculation.getCommuteType() + "," +
                        calculation.getJobInterest() + "," + calculation.getSalarySatisfaction() + "," + calculation.getMedianSalary() + "," +
                        calculation.getRidersArea() + "," + calculation.getAveRidersArea() + "," + calculation.getCommuteTime() + "," +
